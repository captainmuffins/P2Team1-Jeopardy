...
export class GameComponent implements OnInit {
  ...
  getCluesPerCategory() {
    for (let i = 0; i < this.chosenJCategories.length; i++) {
      const curCat = this.chosenJCategories[i];
    ...
    console.log(this.chosenJCategories);
    ...
    this._jeopardyService.getCategory(catId).subscribe({
      next: (data) => {
        ...
        if (data.clues_count > 9) {
          ...
          this.catLoadIdx++;
          if (this.catLoadIdx < this.maxCats) {
            this.retrieveGameData();
          } else {
            console.log(
  ...
  doLoad() {
    let checkedCats = document.querySelectorAll('.selectCatButtons.checked');
    if (checkedCats.length > 0) {
      this.metMinimumCat = true;
      ...
      this.maxProgress = checkedCats.length * 5 + additionalProgress;
      this.attemptData.maxClues = checkedCats.length * 5;
      console.log(this.maxProgress);
      console.log(this.attemptData.maxClues);
      this.gameStart();
    } else {
      // Do Nothing. Display Message
      this.metMinimumCat = false;
    }
    console.log(checkedCats);
  }

  metMinimumCat: boolean = true;

  loadClues() {
    let checkedCats = document.querySelectorAll('.selectCatButtons.checked');

    for (let i = 0; i < checkedCats.length; i++) {
      const selCat = checkedCats[i] as HTMLElement;
      const splitId = selCat.id.split('-') as Array<string>;
      const getCatIndex = parseInt(splitId[1]);
      this.chosenJCategories.push(this.jCategories[getCatIndex]);
    }
