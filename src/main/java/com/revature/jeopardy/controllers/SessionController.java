...
public class SessionController {
	...
	public ResponseEntity<Response> addSession(@RequestBody SessionDTO sessionDTO) {
		...
		try {
			...
			// Optional<Players> playerOptional =
			// playerDAO.findById(sessionDTO.getSessionPlayerfk());
			PlayersSummary ps = playerDAO.findByPlayerId(sessionDTO.getSessionPlayerfk()).get(0);
			Players player = new Players(ps.getPlayerId(), ps.getPlayerUsername(), ps.getPlayerPassword(),
	...
	public ResponseEntity<Response> getAllSession() {
		...
		try {
			...
			for (Session s : sessions) {
				s.getSessionPlayerfk().setPlayerAvatar(null);
			}
